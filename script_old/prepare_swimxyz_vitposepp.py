...
# Consolidated training dataset builder for the currently approved workflow.
# This is the script that turns SwimXYZ labels into a VitPose++ standard
# COCO17 dataset while applying the hypotheses we decided to keep active:
# - Y is flipped from bottom-origin to top-origin
# - eyes and ears are swapped left/right before COCO17 conversion
# - overlay images are generated for every final split sample
# - anomalous bbox jumps are logged for later inspection
...
from prepare_swimxyz_vitposepp_utils import (
    ...
    move_split_images,
    normalize_source_tree_to_converted,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Prepare a SwimXYZ COCO17 dataset and a standard VitPose++ config, "
            "using the current pipeline but swapping left/right ear and eye "
            "keypoints before the SwimXYZ->COCO17 conversion."
        )
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Optional raw SwimXYZ source root. When set, the script first "
            "normalizes source videos/labels into data/intermediate/<dataset>/_converted "
            "and then prepares the final VitPose++ dataset from there."
        ),
    )
    parser.add_argument(
        "--dataset-root",
    ...
    parser.add_argument(
        "--intermediate-root",
        default=DEFAULT_DATASET_ROOT,
        help="Destination root for normalized '_converted' datasets when --source-root is used.",
    )
    parser.add_argument(
        "--dataset-entry",
        action="append",
...
def build_canonical_output_root(
    project_root: Path,
    dataset_root: Path,
    ...
    return (project_root / "data" / "intermediate" / dataset_name / "_train_canonical").resolve()


def swap_head_side_keypoints(row: dict[str, str]) -> dict[str, str]:
    # We keep the original parser unchanged and apply the hypothesis locally
    # here, so the "swap" variant can be compared against the standard mapping
    # without mutating the baseline implementation.
    swapped_row = dict(row)
    for suffix in ("x", "y", "z"):
        left_key = f"LEye.{suffix}"
        right_key = f"REye.{suffix}"
        swapped_row[left_key] = row.get(right_key, "")
        swapped_row[right_key] = row.get(left_key, "")
    for suffix in ("x", "y", "z"):
        left_key = f"LEar.{suffix}"
        right_key = f"REar.{suffix}"
        swapped_row[left_key] = row.get(right_key, "")
        swapped_row[right_key] = row.get(left_key, "")
    return swapped_row
...
def extract_samples_from_entry(
    ...
    # Each accepted frame becomes one COCO-style sample.
    # We log data issues while keeping the rest of the video usable so a small
    # number of bad terminal rows does not invalidate the whole sequence.
    ...
    while True:
        ...
        swapped_row = swap_head_side_keypoints(label_rows[label_index])
...
def prepare_dataset(
    ...
    # The consolidated output is self-contained: images, overlays, COCO JSON,
    # logs and generated config all live under one dataset root.
    ...
    report = {
        ...
        "ear_swap_enabled": True,
        "eye_swap_enabled": True,
    ...
    print(
        "Prepared SwimXYZ dataset for standard VitPose++ with swapped ears: "
...
def main() -> None:
    ...
    if args.source_root and args.dataset_entry:
        raise ValueError(
            "Do not combine --source-root with --dataset-entry. First normalize the source "
            "dataset, then let the script discover entries from the generated '_converted' root."
        )

    if args.source_root:
        source_root = resolve_path(project_root, args.source_root)
        intermediate_root = resolve_path(project_root, args.intermediate_root)
        dataset_roots = normalize_source_tree_to_converted(source_root, intermediate_root)
        if len(dataset_roots) > 1 and args.output_root:
            raise ValueError(
                "When processing multiple normalized source datasets, do not pass a shared "
                "--output-root. Let the script infer one '_train_canonical' directory per dataset."
            )
        for current_dataset_root in dataset_roots:
            output_root = build_canonical_output_root(
                project_root,
    ...
    if args.dataset_entry:
        output_root = build_canonical_output_root(project_root, dataset_root, args.output_root)
