...
TMUX_CMD="cd ${PROJECT_ROOT} && conda run -n ${CONDA_ENV} python script/dataset_preparation-cleaning/prepare_swimxyz_frames_dataset.py --input-root ${INPUT_ROOT_REL} --output-root ${OUTPUT_ROOT_REL} --val-ratio ${VAL_RATIO} --test-ratio ${TEST_RATIO} --copy-mode ${COPY_MODE} --vitpose-work-dir ${VITPOSE_WORK_DIR_REL} --yolo-pose-link-mode ${YOLO_POSE_LINK_MODE} --overwrite 2>&1 | tee ${LOG_REL}"
