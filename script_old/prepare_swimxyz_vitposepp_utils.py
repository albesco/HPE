...
def iter_source_dataset_roots(source_root: Path) -> list[Path]:
    manifest_path = source_root / "manifest.json"
    if manifest_path.is_file():
        return [source_root]

    datasets = sorted(
        child for child in source_root.iterdir() if child.is_dir() and (child / "manifest.json").is_file()
    )
    if datasets:
        return datasets
...
def build_converted_output_root(intermediate_root: Path, dataset_root: Path) -> Path:
    return intermediate_root / dataset_root.name / "_converted"


def build_converted_label_output_path(
    output_root: Path,
    video_stem: str,
    label_filename: str,
) -> tuple[str, str, Path]:
    label_name = Path(label_filename).name
    stem_without_suffix, representation, label_kind = label_name.rsplit("__", 2)
    if stem_without_suffix != video_stem:
        raise ValueError(
            f"Label {label_name} does not match video stem {video_stem}"
        )
...
def normalize_source_dataset_to_converted(
    source_root: Path,
    output_root: Path,
) -> dict[str, int]:
    ...
    for item in items:
        ...
        for label_filename in item.get("label_files", []):
            ...
            representation, label_kind, output_label_path = build_converted_label_output_path(
...
def normalize_source_tree_to_converted(
    source_root: Path,
    intermediate_root: Path,
) -> list[Path]:
    converted_roots: list[Path] = []
    for dataset_root in iter_source_dataset_roots(source_root):
        output_root = build_converted_output_root(intermediate_root, dataset_root)
        summary = normalize_source_dataset_to_converted(dataset_root, output_root)
        converted_roots.append(output_root)
        print(f"Normalized SwimXYZ dataset: {dataset_root.name}")
        print(f"Source root: {dataset_root}")
        print(f"Converted root: {output_root}")
        print(f"Videos copied: {summary['videos']}")
        print(f"Label files copied: {summary['labels']}")
...
def build_keypoints_and_bbox(
    ...
    for body25_name, _ in BODY25_TO_COCO:
        ...
        all_points.append((clipped_x, clipped_y))
        if v > 0.0:
    ...
    candidate_points = visible_points if visible_points else all_points
    if not candidate_points:
        bbox = [0.0, 0.0, float(width), float(height)]
        area = round(float(width) * float(height), 2)
