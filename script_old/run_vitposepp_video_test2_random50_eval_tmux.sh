...
GPU_ID="${GPU_ID:-0}"
DEVICE="${DEVICE:-auto}"
SESSION_NAME="${SESSION_NAME:-vitposepp_video_test2_random50_eval}"

SOURCE_DATASET_ROOT_REL="${SOURCE_DATASET_ROOT_REL:-data/intermediate/Side_above_water_VideoTest2/_train_canonical}"
SUBSET_ROOT_REL="${SUBSET_ROOT_REL:-data/intermediate/Side_above_water_VideoTest2/_train_canonical/subsets/random50_seed20260603}"
...
CONFIG_REL="${CONFIG_REL:-${SUBSET_ROOT_REL}/generated_configs/swimxyz_vitposepp_videotest2_random50_eval.py}"
OUT_DIR_REL="${OUT_DIR_REL:-runs/vitposepp_video_test2_random50_eval_best_AP_epoch_24}"
PREDICTIONS_REL="${PREDICTIONS_REL:-${OUT_DIR_REL}/test_predictions.pkl}"
LOG_REL="${LOG_REL:-${OUT_DIR_REL}/test_eval_stdout.log}"

cd "${PROJECT_ROOT}"

conda run -n "${CONDA_ENV}" python script/prepare_vitposepp_video_test2_subset.py \
  --source-dataset-root "${SOURCE_DATASET_ROOT_REL}" \
  --output-root "${SUBSET_ROOT_REL}" \
  --checkpoint "${CHECKPOINT_REL}" \
  --work-dir "${OUT_DIR_REL}" \
...
echo "Subset root: ${PROJECT_ROOT}/${SUBSET_ROOT_REL}"
