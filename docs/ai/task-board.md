...
## In progress
- VitPose++ training in progress: `vitpose_SUW_frames_20260705` launched via GPU-free tmux trigger session `trigger_vitpose_suw_frames_20260705`, which immediately started tmux session `vitpose_suw_frames_20260705` on `data/intermediate/SUW_frames` using `script/vitpose_training/train_vitpose_frame.sh`; pretrained checkpoint `models/pose/wholebody.pth`, YOLO detector checkpoint `runs/yolo26x-detection_SUW_frames_20260701/weights/best.pt`, `batch-size=1`, `max-epochs=100`; outputs target `runs/vitpose_SUW_frames_20260705/` and `data/output/experiments/vitpose_SUW_frames_20260705/`.
...
- YOLO26x-Pose top-1 retraining in progress: `yolo26x-pose_SUW_frames_20260709_top1` launched via tmux session `train_yolo26x_pose_yolo26x-pose_SUW_frames_20260709_top1` on `data/intermediate/SUW_frames` with `max_det=1`, `min_epochs=20`, `patience=3`, and `min_delta=0.007`; outputs target `runs/yolo26x-pose_SUW_frames_20260709_top1/` and `data/output/experiments/yolo26x-pose_SUW_frames_20260709_top1/`.

## Done
- Updated YOLO26x-Pose training/prediction/evaluation scripts to keep only the highest-confidence detection per image (`max_det=1`) for validation, exported keypoints, and overlays.
- Corrected the YOLO26x-Pose training procedure so the external `metrics/mAP50-95(P)` monitor saves `best_map50_95_pose.pt`, Test evaluation uses that checkpoint, and default stopping is less premature with `patience=8` and `min_epochs=20`.
