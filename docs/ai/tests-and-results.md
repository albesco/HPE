...
| 2026-07-02 | YOLO26x-Detection SUW_frames dataset-label fix | `data/intermediate/SUW_frames/_Yolo26x_detection` | fixed and relaunched | initial run had precision/recall/mAP all `0` because directory symlinks in `images/{train,val,test}` made Ultralytics cache images under `_train_canonical` with zero labels; replaced split image dir symlinks with real dirs containing per-file symlinks, removed stale `_train_canonical/*.cache`, relaunched `yolo26x-detection_SUW_frames_20260701`; new caches read train `6044` boxes and val `1727` boxes. |
| 2026-07-02 | YOLO26x-Detection SUW_frames training | `yolo26x-detection_SUW_frames_20260701` | launched in tmux | session `train_yolo26x_detection_SUW_frames_20260701`; dataset `data/intermediate/SUW_frames`; checkpoint `models/detection/yolo26x.pt`; run directory `runs/yolo26x-detection_SUW_frames_20260701`; `patience=3`, `keep_epoch_ckpts=10`, native Ultralytics early stopping without external `min_delta`; new optional monitor `script/yolo26x_detection_training/monitor_yolo_detection_patience.py` validates with `py_compile`; metrics pending. |
| 2026-07-02 | YOLO26x-Detection script consolidation | `script/yolo26x_detection_training/`, `script/yolo26x_detection_prediction/` | static validation passed | promoted detector training, top-1 prediction/overlay, and YOLO->VitPose evaluation scripts from `script_old/yolo_training/`; `bash -n` passed for the training launcher and `python3 -m py_compile` passed for the copied Python helpers. |
...
No valid full-test metric is currently recorded for the consolidated `YoloVitPose_mAP` pipeline. Before reporting new end-to-end AP, run the operational bridge `script/yolo26x_detection_prediction/evaluate_yolo_vitpose_map.py`.
...
```bash
python -m py_compile script/yolo26x_detection_prediction/evaluate_yolo_vitpose_map.py script/vitpose_training/pose_overlay_utils.py
```
