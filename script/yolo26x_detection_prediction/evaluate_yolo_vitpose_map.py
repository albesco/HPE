...
VITPOSE_TRAINING_DIR = SCRIPT_DIR / "vitpose_training"
for import_dir in (SCRIPT_DIR, VITPOSE_TRAINING_DIR):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default="", help="Exact output directory. Overrides timestamped output-root behavior.")
    parser.add_argument("--keypoints-out", default="", help="Optional exact path for COCO keypoint predictions JSON.")
    parser.add_argument("--metrics-out", default="", help="Optional exact path for metrics-only JSON.")
    parser.add_argument("--summary-out", default="", help="Optional exact path for run summary JSON.")
    parser.add_argument("--overlay-dir", default="", help="Optional exact overlay directory.")
...
def main() -> None:
    ...
    overlay_dir = Path(args.overlay_dir).expanduser().resolve() if args.overlay_dir else run_dir / "overlays"
    ...
    if args.overlay_count < 0:
        overlay_ids = {int(img["id"]) for img in images}
    elif args.overlay_count == 0:
        overlay_ids = set()
    else:
    ...
    for index, img_info in enumerate(images, start=1):
        ...
        if image_id in overlay_ids and (args.overlay_count < 0 or overlay_saved < args.overlay_count):
            save_overlay(
                model=vitpose,
    ...
    results_path = Path(args.keypoints_out).expanduser().resolve() if args.keypoints_out else run_dir / "yolo_vitpose_keypoints_results.json"
    ...
    metrics_path = Path(args.metrics_out).expanduser().resolve() if args.metrics_out else None
    if metrics_path is not None:
        metrics_path.parent.mkdir(parents=True, exist_ok=True)
        metrics_path.write_text(json.dumps(metrics, indent=2, sort_keys=True), encoding="utf-8")

    summary_path = Path(args.summary_out).expanduser().resolve() if args.summary_out else run_dir / "summary.json"
