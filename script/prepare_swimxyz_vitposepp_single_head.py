...
# This is the script that turns SwimXYZ labels into a VitPose++ single-head
...
DEFAULT_WORK_DIR = "runs/vitposepp_single_head_subset_xyz_swap_ears"
...
def build_vitposepp_output_root(
    ...
    return (project_root / "data" / "intermediate" / dataset_name / "_train_vitposepp_swap_ears").resolve()
...
def prepare_dataset(
    ...
    generated_config = output_root / "generated_configs" / "swimxyz_vitposepp_huge_single_head_swap_ears.py"
    ...
    print(
        "Prepared SwimXYZ dataset for single-head VitPose++ with swapped ears: "
...
def main() -> None:
    ...
    if len(dataset_roots) > 1 and args.output_root:
        raise ValueError(
            ...
            "--output-root. Let the script infer one '_train_vitposepp_swap_ears' directory per dataset."
        )
