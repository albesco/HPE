#!/usr/bin/env python3
"""Grid-evaluate YOLO26x-Pose on train/validation splits only.
...
DEFAULT_MODEL = "models/pose/yolo26x-pose.pt"
DEFAULT_OUTPUT_DIR = "runs/grid-eval/yolo26x_pose"
DEFAULT_GRID = {
    "lr0": [0.00067, 0.00100],
    "imgsz": [640, 768],
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a train/val-only YOLO26x-Pose grid-evaluation."
    )
    parser.add_argument("--train-data", help="YOLO pose train images directory.")
    parser.add_argument("--val-data", help="YOLO pose validation images directory.")
    parser.add_argument("--test-data", help="Optional YOLO pose test images directory; recorded but not used for selection.")
    parser.add_argument("--epochs", type=int, default=5)
    parser.add_argument("--patience", type=int, default=5)
    parser.add_argument("--metric", default="mAP50-95", help="Primary keypoint fallback metric alias; default is Keypoint AP@[OKS 0.50:0.95].")
...
def validate_pose_grid(grid: dict[str, list[Any]]) -> None:
    required = {"lr0", "imgsz"}
    missing = sorted(required - set(grid))
    if missing:
        raise ValueError(f"YOLO26x-Pose grid is missing required parameter(s): {', '.join(missing)}")
    unsupported = sorted(set(grid) - required)
    if unsupported:
        raise ValueError(
            "YOLO26x-Pose grid may vary only lr0 and imgsz; "
...
def validate_pose_label(path: Path) -> bool:
    text = path.read_text(encoding="utf-8").strip()
    if not text:
        return False
    for line in text.splitlines():
        if len(line.split()) != 56:
            return False
    return True
...
def validate_split(images_dir: Path, split_name: str) -> dict[str, Any]:
    ...
    for image in images:
        ...
        elif not validate_pose_label(label_path):
            malformed.append(label_path.name)
    if not images:
        raise RuntimeError(f"No images found for {split_name}: {images_dir}")
    if missing:
        raise RuntimeError(f"Missing {len(missing)} {split_name} labels; examples: {', '.join(missing[:5])}")
    if malformed:
        raise RuntimeError(f"Malformed {split_name} pose labels; expected 56 fields. Examples: {', '.join(malformed[:5])}")
...
def write_data_yaml(experiment_dir: Path, train_data: Path, val_data: Path, test_data: Path | None) -> Path:
    ...
    lines.extend(
        [
            "kpt_shape: [17, 3]",
            f"flip_idx: {json.dumps(FLIP_IDX)}",
            "names:",
            "  0: swimmer",
            "kpt_names:",
            "  0:",
            *[f"    - {name}" for name in KPT_NAMES],
            "",
        ]
    )
...
def write_report(experiment_dir: Path, rows: list[dict[str, Any]], best: dict[str, Any] | None, metric_name: str) -> None:
    lines = [
        "# YOLO26x-Pose grid-evaluation",
        "",
        "Purpose: select a YOLO26x-Pose configuration for end-to-end swimmer detection plus COCO17 keypoint estimation.",
        "",
        ...
        "",
        "Selection priority: Keypoint AP@[OKS 0.50:0.95], then AP75 if available, then AR if available, then imgsz=640.",
        "",
        "Optimizer note: `optimizer=AdamW` is emitted intentionally because repo memory records that `optimizer=auto` can ignore or override `lr0`; AdamW matches the historical YOLO26x-Pose grid reference while leaving augmentation, scheduler, losses and inference/NMS at framework defaults.",
        "",
        "Unavailable metrics: AP75, AR, mean per-keypoint error and distal-keypoint metrics are not emitted by standard Ultralytics train `results.csv`; they remain null unless a separate evaluator is added. Values are not inferred.",
        "",
        "## Best Config",
        "",
    ]
    if best is None:
        ...
        lines.extend(
            [
                ...
                f"- Keypoint AP50: `{best.get('keypoint_ap50')}`",
    ...
    for row in rows:
        lines.append(
            ...
            f"{row.get('keypoint_ap75')} | {row.get('keypoint_ar')} | {row.get('box_map50')} | {row.get('box_map50_95')} |"
        )
...
def run_config(
    ...
    write_json(
        run_dir / "args.json",
        {
            ...
            "optimizer_note": "optimizer=AdamW keeps lr0 effective; optimizer=auto is documented in repo memory as able to ignore/override lr0.",
        },
    )
    ...
    if not resolve_path(args.model).is_file() and not resume:
        raise FileNotFoundError(f"Missing YOLO pose model checkpoint: {resolve_path(args.model)}")
...
def main() -> None:
    ...
    write_json(
        experiment_dir / "grid_effective.json",
        {
            ...
            "defaults_not_overridden": "Augmentation, weight decay, scheduler, pose-specific losses, kobj, RLE loss and inference/NMS stay at Ultralytics/model defaults unless emitted in command.txt.",
            "explicit_reference_overrides": {
                "batch": 1,
                "optimizer": "AdamW",
                "save_period": -1,
                "split": "val",
            },
            "optimizer_note": "optimizer=AdamW is set because repo memory records optimizer=auto can ignore lr0; this keeps lr0 effective.",
        },
    )
